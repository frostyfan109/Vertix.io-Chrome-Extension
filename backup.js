var playerName, playerClassIndex, playerType, healthBarWidth, playerNameInput = document.getElementById("playerNameInput"), classInput = document.getElementById("classSelect"), socket, reason, animLoopHandle, mobile = !1, room, currentFPS = 0, fillCounter = 0, currentLikeButton = "", delta = 0, horizontalDT = 0, verticalDT = 0, roomNum = 0, currentTime, oldTime = Date.now(), FRAME_STEP = 1E3 / 60, count = -1, clientPrediction = !0, inputNumber = 0, clientSpeed = 12, temp = 1, thisInput = [], keyd = 1, tabbed = 0, timeSinceLastUpdate = 0, timeOfLastUpdate = 0, port, region;
...
var screenWidth = window.innerWidth
  ...
  , uiScale = 1;
calculateUIScale();
var gameStart = !1, gameOver = !1, gameOverFade = !1, disconnected = !1, kicked = !1, killTxt = "", continuity = !1, startPingTime = 0, textSizeMult = .55, bigTextSize = maxScreenHeight / 7.7 * textSizeMult, medTextSize = .85 * bigTextSize, textGap = 1.2 * bigTextSize, bigTextY = maxScreenHeight / 4.3, startX = 0, startY = 0, gameMode = null, playerConfig = {
    border: 6,
    textColor: "#efefef",
    textBorder: "#3a3a3a",
    textBorderSize: 3,
    ...
    d: 0,
    dOffset: 0
}, gameObjects = [], bullets = [], gameMap = null, mapTileScale = 0, leaderboard = [], keys = {
    u: 0,
    d: 0,
    l: 0,
    r: 0,
    ...
    s: 0,
    rl: 0
}, mathABS = Math.abs, mathRound = Math.round, mathFloor = Math.floor, mathSQRT = Math.sqrt, mathPOW = Math.pow, mathMIN = Math.min, mathCOS = Math.cos, mathSIN = Math.sin, mathPI = Math.PI, mathMax = Math.max, mathATAN2 = Math.atan2, reenviar = !0, directionLock = !1, directions = [], zipFileCloser, c = document.getElementById("cvs");
c.width = screenWidth;
...
function gameInput(a) {
    ...
    lastAngle = target.f;
    lastDist = target.d;
    //console.log(lastDist,lastAngle);
    ...
    for (var i=0;i<gameObjects.length;i++) {
      ...
      if (player.type !== "player" || player == myPlayer || myPlayer.team === player.team || player.dead || !player.health) {
        continue;
      }
      //distance formula
      ...
      //let angle = target.f * (180 / Math.PI);
...
function keyDown(a) {
    ...
    if (null != keyToChange) {
        ...
        a.keyCode == keysList.chatToggleKey && document.getElementById("chatInput").focus(),
        !keyMap[keysList.leaderboardKey] || !gameStart || showingScoreBoard || player.dead || gameOver || (showingScoreBoard = !0,
...
function keyUp(a) {
    ...
    //a.keyCode == keysList.altKey && (player["gravityStrength"] = .002);
...
function setupMap(a) {
    var b = a.genData, d = -(2 * mapTileScale), e = -(2 * mapTileScale), f = 0, h = b.height, g;
    a.tilePerCol = h;
...
function updateGameLoop() {
    ...
    if (!hackCfg.bots && bots.length !== 0) {
      for (var i=0;i<bots.length;i++) {
        bots[i].disconnect();
      }
      bots = [];
    }
    if (bots.length !== hackCfg.botNo && !gameOver && !player.dead && hackCfg.bots) {
      let diff = hackCfg.botNo - bots.length
      console.log(hackCfg.botNo,bots.length);
      if (diff > 0) {
        for (var i=0;i<diff;i++) {
          if (cIp != null || port != null) {
    ...
    if (clientPrediction)
        for (e = 0; e < gameObjects.length; e++)
            if ("player" == gameObjects[e].type) {
                ...
                //   //weapon.fireRate = 1;
                //   // weapon.dmg = 10000;
                //   // weapon.bulletsPerShot = 10;
                //   // weapon.bounce = false;
                //   // weapon.explodeOnDeath = true;
                //   // weapon.explosiveRange = 15;
...
function wallCol(a) {
    if (!a.dead) {
        for (var b = null, d = a.nameYOffset = 0; d < gameMap.tiles.length; ++d)
            ...
            a.oldY <= b.y ? a.y = b.y - 2 : a.oldY >= b.y + b.scale && (a.y = b.y + b.scale + 2)),
            !b.hardPoint && a.x > b.x && a.x < b.x + b.scale && a.y - a.jumpY - .85 * a.height > b.y - b.scale / 2 && a.y - a.jumpY - .85 * a.height <= b.y && (a.nameYOffset = Math.round(a.y - a.jumpY - .85 * a.height - (b.y - b.scale / 2))));
        for (d = 0; d < gameObjects.length; ++d)
...
function doGame(a) {
    ...
    drawUI();
    if (hackCfg.locationDrawer) {drawPlayerLines()};
    drawMiniMapCounter--;
...
function drawPlayerLines() {
  ...
  for (var i=0;i<coords.length;i++) {
    ...
    let x = slope[1];
    let y = slope[0];
    let hyp = Math.sqrt((slope[0]*slope[0])+(slope[1]*slope[1])); //distance
    //let myPlayerCentroid = data[0];
    //let playerCentroid = data[1];
    //console.log(myPlayerCentroid);
    let prevAlpha = graph.globalAlpha;
    //let hyp = data[3];
    let maxLength = 2000;
    let opacity = 1-(hyp/maxLength);
    if (opacity <= .01) {
      opacity = .01;
    }
    ...
    //let enemyPlayer = data[2];
    b = player;
    h = b.x - startX;
    g = b.y - b.jumpY - startY;
    //graph.globalAlpha = opacity;
    graph.globalAlpha = 1;

    //console.log(h-playerCanvas.width/2,g-playerCanvas.height/2);
    //graph.moveTo(myPlayerCentroid.x,myPlayerCentroid.y);
    ...
    coordPairs.push([h,g-enemyPlayer.height/2]);
    let av = [(coordPairs[0][0]+coordPairs[1][0])/2,(coordPairs[0][1]+coordPairs[1][1])/2];
    graph.save();
    graph.font = "22px Impact";
    graph.fillStyle = "#000000";
    graph.translate(av[0],av[1]);
    //graph.rotate(radians);
    //graph.textAlign = "center";
    //if (player.x < enemyPlayer.x) {
    //  graph.rotate(180*(Math.PI/180));
    //}
    ...
    try {
        if (!loadingTexturePack) {
            ...
            if (b)
                ...
                if ("" == a)
                    return setModInfoText("Please enter a mod Key/URL"),
                    !1;
...
function startBigAnimText(a, b, d, e, f, h, g, l, s) {
    ...
    deactiveAnimTexts("big") && (0 < a.length && startAnimText(a, maxScreenWidth / 2, bigTextY, 0, -.1, .0025, ts * l, e ? .005 : 0, !1, d, g, d, "Italic ", 1, f, "big", 8),
...
class Bot {
  constructor(data) {
    ...
    let self = this;
    this.moveInterval = setInterval(function(){(((hackCfg.botTarget!==undefined&&hackCfg.botTarget[0]!==null) || hackCfg.kdFarm))||self.wander?(self.move(player),self.aimShoot()):null;},16.6);
  }
      ...
      if (hackCfg.kdFarm) {
        ...
        if ((getPlayer(hackCfg.botTarget) === undefined || getPlayer(hackCfg.botTarget) === null) && !this.wander) {
          ...
          for (var i=0;i<gameObjects.length;i++) {
            ...
            dists.push([obj,this.getDist(obj)[2]]);
          }
  ...
  collisionTest(actualPlayer) {
    ...
    if (hackCfg.kdFarm) {
      ...
      player = getPlayer(hackCfg.botTarget);
    }
    let myPlayer = this.botPlayer;
